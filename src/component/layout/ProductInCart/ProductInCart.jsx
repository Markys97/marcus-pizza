...
import { incrementQt,decrementQt,removeProductInCart } from '../../../redux/slices/product';
function ProductInCart({productData}) {
    const dispatch = useDispatch();
    const {id,qt,size,type,price} = productData
    const listProduct = useSelector(state => state.product.listProduct)
    const currentProduct = listProduct.find(item=> item.id ===id)
    const {name,img}= currentProduct
    const totalPrice = price * qt

    const incrementQuantityProduct = ()=>{
      dispatch(incrementQt(id))

    }
    const decrementQuantityProduct = ()=>{
        dispatch(decrementQt(id))
        if(qt ===1){
            dispatch(removeProductInCart(id))
        }
    }

    const removeProduct = ()=> dispatch(removeProductInCart(id))
        ...
        <div className="product-in-cart__row">
            ...
            <div className="product-in-cart__info">
                <div className="product-in-cart__img">
                    <img src={`/images/products/${img}` }alt={`pizza ${name}`} />
                </div>
                <div className="product-in-cart__detail">
                    <div className="product-in-cart__name">{name}</div>
                    <div className="product-in-cart__params">{type}, {size}</div>
                </div>
            </div>
            ...
            <div className="product-in-cart__control">
                <div className="product-in-cart__quantity">
                    ...
                    <ControlButton handlerClick={decrementQuantityProduct}>
                    ...
                    <div className="product-in-cart__number">{qt}</div>

                    <ControlButton  handlerClick={incrementQuantityProduct} >
                ...
                <div className="product-in-cart__price">{totalPrice} ₽ </div>

               
                <ControlButton handlerClick={removeProduct} type="light">
